...
These studies use the $WW$ samples as signal and the dijet $gg$
samples to model the QCD background, as described previously in Section~\ref{sec:samples}. Whilst only gluonic backgrounds
...
In each of the three \pt slices considered jets are reconstructed using the
\antikt algorithm with distance parameter R=0.4, 0.8 and 1.2, as
described in Section~\ref{sec:samples}. They then have various
grooming approaches applied as described in
Section~\ref{sec:observables}. 
{\bf (ED: Probably better if some of the information from
  Sections~\ref{sec:samples} and~\ref{sec:observables} is brought into
