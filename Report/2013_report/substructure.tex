...
We organize the algorithms into four categories:~clustering algorithms,  grooming algorithms, tagging algorithms, and other substructure variables that incorporate information about the shape of radiation inside the jet. We emphasize that these distinctions are arbitrary:~for example, some of the ``grooming'' algorithms (such as trimming and pruning) as well as $N$-subjettiness can be used in a ``tagging'' capacity. This ambiguity is particularly pronounced in multivariate analyses, such as the ones we present here, since a single variable can act in different roles depending on which other variables it is combined with. Therefore, the following classification is  intended only to give an approximate organization of the variables, rather than as a definitive taxonomy.
